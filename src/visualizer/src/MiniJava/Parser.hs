...
parseFromText :: Text -> Config -> IO (Either (ParseErrorBundle Text Void) MiniJavaAST)
parseFromText program cfg = do
  return $ runIdentity $ R.runReaderT (runParserT miniJavaP "hello.world" program) cfg
